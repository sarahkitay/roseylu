...
def test_review_one_handles_markdown_fenced_json(monkeypatch):
    ...
    verdict = trl.review_one("q", "a", "anthropic", "fake-model")
...
def test_checkpoint_does_not_advance_past_a_failing_review(isolated_paths, monkeypatch):
    ...
    def flaky_review(child, reply, backend, model):
        if child == "q2":
    ...
    n = trl.process_new_interactions("anthropic", "fake-model")
    ...
    monkeypatch.setattr(trl, "review_one", lambda child, reply, backend, model: {"appropriate": True, "quality": 5, "corrected_reply": None})
    n2 = trl.process_new_interactions("anthropic", "fake-model")
